...
async function renderSlots(){
  ...
  if(!isPast){
    ...
    html+=`<div class="book-form">
      ...
      <div class="bf-field"><label>Matéria / observação (opcional)</label><input id="bfObs" placeholder="Ex.: Ciências — levar cabo HDMI" autocomplete="off"></div>
    ...
    html+=resvs.map(v=>{
      ...
      return `<div class="rv-item ${mine?'mine':''}">
        ...
        <div class="rv-info">
          ...
          <div class="rv-sub">${[TURNO_LABEL[v.turno],esc(v.turma||v.obs||'Reservado')].filter(Boolean).join(' · ')}</div>
        </div>
