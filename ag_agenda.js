...
function renderChips(){
  ...
  RESOURCES.forEach(r=>{
    ...
    b.onclick=()=>{state.resource=r.id;state.formOpen=true;renderChips();renderSlots();};
...
function renderWeek(){
  ...
  for(let i=0;i<6;i++){
    ...
    b.onclick=()=>{state.date=ds;state.formOpen=true;renderWeek();renderSlots();};
    strip.appendChild(b);
  }
}
function renderTurnos(){
  document.querySelectorAll('#turnos button').forEach(b=>{
    ...
    /* Só marca o turno escolhido — NÃO reconstrói o formulário,
       para não apagar o horário/turma que o professor já preencheu. */
    b.onclick=()=>{
      state.turno=b.dataset.t;
      document.querySelectorAll('#turnos button').forEach(x=>x.classList.toggle('active',x.dataset.t===state.turno));
    };
  });
}
async function renderSlots(){
  ...
  const resvs=[];
  for(const k of keys){ const raw=await sget(k); if(raw){ const v=JSON.parse(raw); if(reservaExpirou(date,v.end)) continue; v._id=k.split(':')[3]; resvs.push(v); } }
  resvs.sort((a,b)=> a.start<b.start?-1:1);
  let html='';
  if(!isPast && state.formOpen){
    const turmaOpts = ['<option value="">Selecione a turma…</option>']
      .concat(TURMAS.map(t=>`<option value="${esc(t)}">${esc(t)}</option>`)).join('');
    html+=`<div class="book-form">
      ...
      <div class="bf-field"><label>Turma</label><select id="bfTurma">${turmaOpts}</select></div>
      <div class="bf-field"><label>Disciplina</label><input id="bfDisc" placeholder="Ex.: Matemática" autocomplete="off"></div>
    ...
    /* Após reservar, o formulário some. Para reservar de novo, é só tocar aqui. */
    html+=`<button class="bf-btn" id="bfNova">+ Nova reserva</button>`;
  }else{
  ...
  if(!resvs.length){
    ...
    html+=resvs.map(v=>{
      ...
      const canCancel=(mine||isAdmin)&&!isPast; // só o dono (ou o admin) pode excluir
      return `<div class="rv-item ${mine?'mine':''}">
        ...
        <div class="rv-info">
          ...
          <div class="rv-sub">${[esc(v.disc),esc(v.turma),TURNO_LABEL[v.turno]].filter(Boolean).join(' · ')||esc(v.obs||'Reservado')}</div>
        </div>
  ...
  box.innerHTML=html;
  if(!isPast){
    const btn=$('#bfReservar'); if(btn) btn.onclick=()=>tryReserve(r,date);
    const nova=$('#bfNova'); if(nova) nova.onclick=()=>{ state.formOpen=true; renderSlots(); };
  }
