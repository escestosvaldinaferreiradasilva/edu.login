...
/* ---------- Armazenamento ----------
   ...
   1) SUPABASE (se configurado em ag_config.js) — banco central: todos veem as mesmas reservas.
      ...
      · sessão    → sempre local no aparelho (localStorage)
   2) window.storage (pré-visualização dentro do Claude)
   3) localStorage do aparelho
   4) memória temporária                                                     */
const hasStore = (typeof window!=='undefined' && window.storage);
const mem = {};
const LS = (function(){ try{ var t='__eeofs_t'; localStorage.setItem(t,'1'); localStorage.removeItem(t); return localStorage; }catch(e){ return null; } })();
...
async function sset(k,v,shared=true){
  ...
  const kk=pfx(k,shared);
  if(LS){ try{ LS.setItem(kk,v); }catch(e){ mem[kk]=v; } } else { mem[kk]=v; }
...
async function sdel(k,shared=true){
  ...
  const kk=pfx(k,shared);
  if(LS){ LS.removeItem(kk); } else { delete mem[kk]; }
}
...
async function slist(prefix,shared=true){
  ...
  if(LS){ for(let i=0;i<LS.length;i++){ const key=LS.key(i); if(key&&key.indexOf(full)===0) out.push(key.slice(base.length)); } }
  else { for(const key of Object.keys(mem)){ if(key.indexOf(full)===0) out.push(key.slice(base.length)); } }
  return out;
}
