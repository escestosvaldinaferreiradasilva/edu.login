...
async function sget(k,shared=true){
  ...
  if(k==='session'){
    try{ return localStorage.getItem("eeofs:session"); }catch(e){ return mem['session']||null; }
  }
  if(sb && shared){
...
async function sset(k,v,shared=true){
  /* sessão: sempre no sessionStorage local (nunca vai ao Supabase) */
  if(k==='session'){
    try{ localStorage.setItem("eeofs:session",v); }catch(e){ mem['session']=v; }
...
async function sdel(k,shared=true){
  ...
  if(k==='session'){
    try{ localStorage.removeItem("eeofs:session"); }catch(e){ delete mem['session']; }
    return;
  }
  if(sb && shared){
    const info=parseKey(k);
...
/* ---------- Expiração automática de reservas ----------
   Regra: a reserva NÃO pode ser cancelada/editada por ninguém.
   Ela some sozinha assim que o horário de término é atingido:
   · dias passados  → expirada
   · dia futuro     → não expirada (fica travada, visível)
   · dia de hoje    → expira quando a hora atual alcança o fim (v.end)   */
function reservaExpirou(dateStr, endHHMM){
  if(!dateStr) return false;
  if(dateStr < todayStr) return true;    // qualquer dia anterior a hoje
  if(dateStr > todayStr) return false;   // dia futuro
  if(!endHHMM) return false;             // hoje, sem fim definido → mantém
  const now=new Date();
  const cur=String(now.getHours()).padStart(2,'0')+':'+String(now.getMinutes()).padStart(2,'0');
  return cur >= endHHMM;                  // encerra ao atingir o fim do horário
}
/* Limpeza silenciosa dos dias já passados (uma consulta, sem toast de erro).
   As de hoje que expiram por horário somem da tela na hora e são apagadas
   no dia seguinte por esta rotina. */
let _purgeFeito=false;
async function purgeDiasPassados(){
  if(_purgeFeito) return; _purgeFeito=true;
  try{
    if(sb){ await sb.from('reservas').delete().lt('data', todayStr); }
  }catch(e){ /* silencioso: nunca incomoda o usuário */ }
}
