...
async function tryReserve(r,date){
  ...
  const disc=($('#bfDisc')?$('#bfDisc').value.trim():'');
  ...
  try{
    ...
    await sset(`resv:${r.id}:${date}:${id}`, JSON.stringify({start,end,by:state.user.login,teacher:state.user.name,turno,turma,disc,obs,createdAt:Date.now()}));
    toast('Reserva confirmada ✓');
    state.formOpen=false;              // some o formulário; volta com "+ Nova reserva"
  ...
  list.forEach(it=>{
    ...
    row.innerHTML=`
      ...
      <div class="mi">
        ...
        <div class="t2">${fmtLong(it.date)} · ${who}${it.disc?' · '+esc(it.disc):''}${it.turma?' · '+esc(it.turma):''}${it.turno?' · '+TURNO_LABEL[it.turno]:''}</div>
      </div>
