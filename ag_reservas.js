...
async function tryReserve(r,date){
  ...
  try{
    ...
    await sset(`resv:${r.id}:${date}:${id}`, JSON.stringify({start,end,by:state.user.login,teacher:state.user.name,turno,turma,obs,createdAt:Date.now()}));
  ...
  list.forEach(it=>{
    ...
    row.innerHTML=`
      ...
      <div class="mi">
        ...
        <div class="t2">${fmtLong(it.date)} · ${who}${it.turno?' · '+TURNO_LABEL[it.turno]:''}${it.turma?' · '+esc(it.turma):''}</div>
      </div>
